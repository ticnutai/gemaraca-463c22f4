...
/**
 * ציטוט שאינו תלמוד בבלי, למרות ששם המסכת זהה — ירושלמי או תוספתא.
 *
...
/**
 * פרק ומשנה או פרק והלכה — לעולם לא דף. שתי צורות:
 * מקוצרת ("פ״י מ״א", "פ״ה ה״ו") ומילולית ("פרק ט הלכה יא", "פרק ב משנה ה").
 * הצורה המילולית מופיעה בעיקר בציטוטי תוספתא ומשנה.
 */
export const isPerekHalacha = (raw: string) =>
  /פ['׳"״][א-ת]['׳"״]?\s*[,;]?\s*[המ]['׳"״][א-ת]/.test(raw)
  || /פרק\s+[א-ת'׳"״]{1,5}\s*[,;]?\s*(?:משנה|הלכה)\s+[א-ת'׳"״]{1,5}/.test(raw);
