...
for (const r of rows) {
  ...
  const before = at > 0 ? ctx.slice(Math.max(0, at - 40), at) : '';

  const ph = raw.match(perekHalacha);
  const isMishna = ph && ph[2] === 'מ';
  const yeru = (precededByYerushalmi(before) || /ירושלמי|ירוש['׳]/.test(raw)) && !looksLikeDaf(raw);
  if (!yeru && !ph) continue;
...
found.slice(0, 10).forEach((f) => console.log(
  `   ${f.row.tractate} ${f.row.daf}${f.row.amud ?? ''} (${f.row.source}) ← "${flat(f.row.raw_reference)}"  ⇒  ${f.corpus} ${f.row.tractate} פרק ${f.perek} ${f.corpus === 'משנה' ? 'משנה' : 'הלכה'} ${f.halacha}`,
));
...
const label = (f) => `${f.corpus}, ${f.row.tractate}, פרק ${f.perek}, ${f.corpus === 'משנה' ? 'משנה' : 'הלכה'} ${f.halacha}`;
const inserts = found.filter((f) => f.perek).map((f) => ({
  psak_din_id: f.row.psak_din_id,
  corpus: f.corpus,
  book: f.row.tractate,
  section: `פרק ${f.perek}`,
  subsection: `${f.corpus === 'משנה' ? 'משנה' : 'הלכה'} ${f.halacha}`,
  display: label(f),
  raw_path: `${f.corpus} ← ${f.row.tractate} ← פרק ${f.perek} ← ${f.corpus === 'משנה' ? 'משנה' : 'הלכה'} ${f.halacha}`,
  source: f.row.source,
